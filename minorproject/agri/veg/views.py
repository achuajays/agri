...
def regi(request):
    if(request.method == "POST"):
        ...
        cp=request.POST['cp']
        if(p==cp):
            a= reg.objects.filter(un=un).first()
            if(a is not  None):
                print("alreready gistred ")
                return render (request,'reg.html',{'key':"alreready gistred "})
            else:
                reg(un=un,em=em,p=p).save()
               
                return redirect(login)
        else:
            return render (request,'reg.html',{'key':"incorrect password"})
...
def login(request):
    if(request.method=="POST"):
        un=request.POST['un']
        p=request.POST['p']
        aa=reg.objects.filter(un=un,p=p).first()
